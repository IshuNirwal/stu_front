...
form.addEventListener('submit', function (e) {
  ...
  else if(employment.value !== 'Salaried') { setError('employment', 'Only salaried employees are eligible.'); ok = false; }
  ...
  else if (Number(salary) < 25000) { setError('salary', 'Salary should be greater than ₹25k'); ok = false; }
...
fetch("http://salarytopup.in/api/offers_page/save_query", {
    method: "POST",
    headers: {
  ...
  if(successFlag===1){
    ...
    var countdown = setInterval(function() {
      ...
      if (seconds <= 0) {
        ...
        window.location.href='https://salarytopup.in/apply-now?mobile=' + window.btoa(mobile) + '&name=' + window.btoa(name) + '&salary=' + window.btoa(salary) + '&employment=' + window.btoa(employment.value);
      }
    }, 1000);
  }
});
